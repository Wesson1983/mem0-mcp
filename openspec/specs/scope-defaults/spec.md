...
`agent_id`. The rule that `user_id` is auto-filled only when neither `agent_id`
nor `run_id` is supplied by the caller SHALL be preserved, evaluated against the
caller-supplied values before default injection.
...
- **THEN** the memory is stored with `agent_id="project-A"` and
  `run_id="session-1"` and `user_id` is NOT auto-filled (the rule suppresses
  user_id when any of agent_id/run_id is present, and agent_id is present after
  default injection)
...
applied to `default_user_id` (see the "Default user_id precedence aligned to
env-wins" requirement below).
...
session-config `default_user_id` with a warning logged on conflict, matching the
precedence rule applied to `default_agent_id`, `mem0_api_key`, and `base_url`.
All four resolved fields SHALL share one precedence rule.
...
default-resolution mechanism. Agent-scoping behavior SHALL be identical to a
deployment that has no default agent scope configured. (The `default_user_id`
precedence rule is a separate, documented exception; see the "Default user_id
...
- **THEN** `add_memory`, `get_memories`, `search_memories`, and
  `delete_all_memories` inject no `agent_id`, matching behavior with no default
  agent scope configured
